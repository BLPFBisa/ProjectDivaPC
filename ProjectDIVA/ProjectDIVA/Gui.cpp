...
{
	title->Draw();
	graphEngine.SetFont(StringTable(56).c_str(),13,false,FW_BOLD);
...
void ListBox::AddItem(int string_index,int style)
{
	...
	btns->GetButton(btns->GetButtonNum()-1)->SetText(string_index,UserFont(StringTable(56).c_str(),12,false,FW_BOLD),MakeRect(16,8,0,0));
}
...
void ListBox::AddItemString(string text,int style)
{
	...
	btns->GetButton(btns->GetButtonNum()-1)->SetTextString(text,UserFont(StringTable(56).c_str(),12,false,FW_BOLD),MakeRect(16,8,0,0));
}
...
void MainMenu::Draw()
{
	...
	{
		mainlb->Draw();
		graphEngine.SetFont(StringTable(56).c_str(),12);
	...
	else if(MainMenuState==MAINST_FREE||MainMenuState==MAINST_SELECTSONG)
	{
		...
		if(MainMenuState==MAINST_FREE)
		{
			if(songlb->btns->GetButtonNum()>0)
			{
				...
				graphEngine.DrawText(StringTable(29),MakeRect(0,171,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				...
				graphEngine.DrawText(StringTable(30),MakeRect(0,184,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				...
				graphEngine.DrawText(StringTable(31),MakeRect(0,197,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				...
				graphEngine.DrawText(StringTable(32),MakeRect(0,210,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				...
				graphEngine.DrawText(StringTable(33),MakeRect(0,223,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
	...
	else if(MainMenuState==MAINST_CONFIG)
	{
		configlb->Draw();
		graphEngine.SetFont(StringTable(56).c_str(),12);
		if(configDetailWords_string.empty())
			graphEngine.DrawText(StringTable(configDetailWords_index),306,85,D3DXCOLOR(D3DCOLOR_ARGB((*mainDetail->efvct.a),255,255,255)));
		else
			graphEngine.DrawText(configDetailWords_string,306,85,D3DXCOLOR(D3DCOLOR_ARGB((*mainDetail->efvct.a),255,255,255)));
	}
}
...
{
	btns->Draw();
	graphEngine.SetFont(StringTable(56).c_str(),14);
	int talpha=255;
	if(alphaHook)
		talpha=(*alphaHook);
	for(int i=0;i<5;i++)
...
void GUIMessageBox::Draw()
{
	...
	if(prepareState==3)
	{
		...
		if(_basicInfo!=40)
		{
			graphEngine.SetFont(StringTable(56).c_str(),fontSize);
		...
		if(_option.size()!=0)
		{
			for(int i=0;i<_option.size();i++)
			{
				...
				graphEngine.DrawText(StringTable(_option[i]), MakeRect(drawRect.left,tpos,drawRect.right-drawRect.left,fontSize), DT_CENTER,
					D3DCOLOR_ARGB(255,255,255,255));
				tpos+=fontSize;
			}
		}
