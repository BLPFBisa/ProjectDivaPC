...
HRESULT VideoEngine::InitDShowTextureRenderer(string filename)
{
	...
	{
		Msg(TEXT("IGraphBuilder->RenderFile() failed!  hr=0x%x"), hr);
		return hr;
	}
...
HRESULT VideoEngine::Play(string filename, bool setTime)
{
	HRESULT hr = S_OK;
	if(play)
		Stop();
...
void VideoEngine::Update()
{
	...
	/*
	if(bAlarm&&n_alarmed==0)//
	{
		LONGLONG tm;
		pNewClock->GetTime(&tm);
		//tm+=pNewClock->tm_offset+4000000;
		tm = tm + pNewClock->tm_offset + 10000000;
		//m_pMS->SetPositions(&tm,AM_SEEKING_AbsolutePositioning,NULL,AM_SEEKING_NoPositioning);
		m_pMP->put_CurrentPosition(tm*1.0e-7);
		bSynchronize = 100;
		//pNewClock->SetTimeOffset();
		//m_pMC->Run();
		bAlarm = false;
