...
void EffectMana::AddEffectNotePress(int x,int y,PLAYSTATE id)
{
	if(id==PLAYSTATE::SAFE||id==PLAYSTATE::FINE||id==PLAYSTATE::COOL)
	{
		effects.push_back((Effect_Base*)(new Effect_SingleFrame(x,y,rhyme_param,
			(GAMEDATA.GetCombo()<100)?effect_note_press_rhyme1:effect_note_press_rhyme2)));
	...
	if(id==PLAYSTATE::COOL)
		effects.push_back((Effect_Base*)(new Effect_MultipleFrame(x,y,16,
			(GAMEDATA.GetCombo()<100)?effect_note_press_burst1:effect_note_press_burst2)));
...
{
	_nowScore = score;
	numberMana.Draw(_nowDrawScore,MakeRect(371,0,0,0),NumberStyle::SCORE,NumberDwFlags::DWFLAG_LEFT,D3DCOLOR_ARGB(255,255,255,255),0,7);
}
