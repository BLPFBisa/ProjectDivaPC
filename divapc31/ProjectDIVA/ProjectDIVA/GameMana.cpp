...
int NoteMana::OnKeyEvent(UINT type, bool bKeyDown, LONGLONG dwTime)
{
		...
		if(best_ptr!=unit.end())
		{
			...
			{	// pressed correctly
				if(best_ptr_time<=gameini.COOL || GAMEDATA.gamemode==GAME_MODE_AUTO)
				{
					...
					GAMEDATA.ModifyHP(gameini.HP_ADD), GAMEDATA.cool_cnt++, GAMEDATA.combo++, GAMEDATA.score += gameini.SCORE_COOL + ((GAMEDATA.combo>50?50:GAMEDATA.combo)/10)*50, best_ptr->Presh(COOL), gameui.SetState(COOL);
					GAMEDATA.maxScoreTempCombo++;
					GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
					if(GAMEDATA.chanceTime)
					{
				...
				{
					GAMEDATA.ModifyHP(gameini.HP_ADD), GAMEDATA.fine_cnt++, GAMEDATA.combo++, GAMEDATA.score += gameini.SCORE_FINE + ((GAMEDATA.combo>50?50:GAMEDATA.combo)/10)*50, best_ptr->Presh(FINE), gameui.SetState(FINE);
					GAMEDATA.maxScoreTempCombo++;
					GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
					if(GAMEDATA.chanceTime)
					{
				...
				{
					GAMEDATA.ModifyHP(gameini.HP_SAFE), GAMEDATA.score += gameini.SCORE_SAFE, GAMEDATA.safe_cnt++, GAMEDATA.combo=0, best_ptr->Presh(SAFE), gameui.SetState(SAFE);
					GAMEDATA.maxScoreTempCombo++;
					GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
					if(GAMEDATA.chanceTime)
					{
				...
				{
					//effectMana.AddEffectResetFlame(ptr->type);
					GAMEDATA.ModifyHP(gameini.HP_SAD), GAMEDATA.score += gameini.SCORE_SAD, GAMEDATA.sad_cnt++, GAMEDATA.combo=0, best_ptr->Presh(SAD), gameui.SetState(SAD);
					GAMEDATA.maxScoreTempCombo++;
					GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
					if(GAMEDATA.chanceTime)
					{
			...
			{	// pressed wrongly
				GAMEDATA.ModifyHP(gameini.HP_DEC), GAMEDATA.combo = 0, GAMEDATA.worst_cnt++, numberMana.SetCombo(0,best_ptr->note.x,best_ptr->note.y),best_ptr->Presh(WORST),gameui.SetState(WORST);
				GAMEDATA.maxScoreTempCombo++;
				GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
				if(GAMEDATA.chanceTime)
				{
	...
	{	
		...
		if(ptr!=unit.end())
		{
			double restTime = fabs(ptr->duration-dwTime-gameini.delay);
			soundEngine->PlayHit(1);
			...
			{
				GAMEDATA.ModifyHP(gameini.HP_SAFE), GAMEDATA.score += gameini.SCORE_SAFE, GAMEDATA.safe_cnt++, GAMEDATA.combo=0, ptr->Presh(SAFE), gameui.SetState(SAFE);
				GAMEDATA.maxScoreTempCombo++;
				GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
				if(GAMEDATA.chanceTime)
				{
			...
			{
				GAMEDATA.ModifyHP(gameini.HP_SAD), GAMEDATA.score += gameini.SCORE_SAD, GAMEDATA.sad_cnt++, GAMEDATA.combo=0, ptr->Presh(SAD), gameui.SetState(SAD);
				GAMEDATA.maxScoreTempCombo++;
				GAMEDATA.maxScoreCanGet += gameini.SCORE_COOL + ((GAMEDATA.maxScoreTempCombo>50?50:GAMEDATA.maxScoreTempCombo)/10)*50;
				if(GAMEDATA.chanceTime)
				{
...
void GameUI::SetComboStar()
{
	...
	combo_star->efvct.AddEffect(new GUIEF_Alpha(white->efvct.a,5,255));
	...
	RECT middleRect = {X-width, Y-width, X+width, Y+width};
...
void GameUI::Draw()
{
	...
	{	// draw combo gauge
		int combo_index = 12-GAMEDATA.GetCombo()*12/100;
...
void GameCore::Run(LpFrame _nowFrame)
{
	...
	for(int i = 0; i < BGSNum; i++)
	{
		if(_nowFrame->_BGM[i]>=0){
			...
			//soundEngine->SetSampleRate(_notemana.speed_factor);
		}
	}
...
void GameCore::Update(LONGLONG dwTime)
{
	...
	if(!paused)
	{
		...
		// Play new notes' sound and video
		while(noteTime <= totalTime)
		{
			LpFrame pFrame = _notemap.GetFrame();
			Run(pFrame);
			if(pFrame)
				noteTime = pFrame->getTimePosition(),_notemap.NextFrame();
			else
				break;
		}
		...
		if(nowFrame==NULL)
			Finish(), ifclear = (GAMEDATA.hp>0 && !cheatDie);
...
void GameCore::ChangePlaySpeed(double deltaExp)
{
	base::set_speed_factor = _notemana.speed_factor*pow(2.0, deltaExp);
...
{
	_notemana.pitch *= pow(2.0, deltaExp);
	to_integer_point(_notemana.pitch);
	
	if(soundEngine->SetPitch(_notemana.pitch)){
		char msg[128];
		sprintf(msg,"%s : x%.4g", StringTable(109).c_str(), _notemana.pitch);
		ShowMessage(msg,1,0,32);
	}
	/*
	soundEngine->pitch_factor = _notemana.pitch;
	char msg[128];
	sprintf(msg,"%s : x%.4g", StringTable(109).c_str(), _notemana.pitch);
	ShowMessage(msg,1,0,32);*/
}
void GameCore::ChangeTempo(double deltaExp)
{
	base::set_speed_factor = _notemana.speed_factor*pow(2.0, deltaExp);
	to_integer_point(base::set_speed_factor);
	char msg[128];
	sprintf(msg,"%s : x%.4g", StringTable(110).c_str(), base::set_speed_factor);
	ShowMessage(msg,1,0,32);
}
...
void GameCore::OnKeyEvent(UINT nChar, bool bKeyDown)
{
	...
	if(bKeyDown){
		...
		//case VK_F9:		ChangeTempo(-1.0/8.0);									break;
		//case VK_F10:	ChangeTempo(+1.0/8.0);									break;
		case VK_F11:	ChangePitch(-1.0/8.0);									break;
		case VK_F12:	ChangePitch(+1.0/8.0);									break;
		case VK_NEXT:	ChangeMusicVolume(-0.25);								break;
		case VK_PRIOR:	ChangeMusicVolume(+0.25);								break;
		case VK_OEM_PLUS:
		case VK_ADD:		ChangePlaySpeed(+1.0/8.0);							break;
		case VK_OEM_MINUS:
		case VK_SUBTRACT:	ChangePlaySpeed(-1.0/8.0);							break;
		}
	}
}
