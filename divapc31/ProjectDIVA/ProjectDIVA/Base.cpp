...
namespace base
{
	...
	char Name[] = "Project DIVA PC 3.3b (HD v1.3b)";
	...
	void MainLoop()
	{
		...
		if(dwTime>=timeDelta)
		{
			...
			if(set_speed_factor!=0){
				if(systemIni.PitchShiftDSP){
					if(!soundEngine->SetTempo(set_speed_factor))
						ShowMessage("Error: SetTempo failed!",1,0,32);
				}else{
					if(!soundEngine->SetSampleRate(set_speed_factor))
						ShowMessage("Error: SetSampleRate failed!",1,0,32);
				}
			}
