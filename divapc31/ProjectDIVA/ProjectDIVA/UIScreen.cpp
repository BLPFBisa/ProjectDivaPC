...
void menu_mainlb_OnButtonClick(int id)
{
	...
	case 5:
		soundEngine->PlaySound("gamedata\\Select.wav",systemIni.sndVolume);
		MessageBox(base::hwnd, StringTable(102).c_str(), StringTable(103).c_str(), MB_OK);
		break;
	case 6:
...
void RefreshConfigDetail()
{
	...
	case 10:menu->configDetailWords_index = 88;	break;
	case 11:
...
void menu_configlb_RefreshDetail()
{
	...
	menu->configlb->btns->GetButton(10)->text2_string = StringTable(56);
	...
	menu->configlb->btns->GetButton(14)->text2_string = StringTable(systemIni.PitchShiftDSP?64:65);
	...
	if(systemIni.keyHook==1 && !base::keyboard_thread_id)
		MessageBox(hwnd,StringTable(72).c_str(),NULL,MB_OK|MB_ICONEXCLAMATION);
}
...
void menu_cgmenu_load()
{
	...
	soundEngine->PlayMusic("gamedata\\cg.mp3",0,true);
}
...
void menu_ListBoxGetOutOver()
{
	...
	{
		menu->songlb->GetIn();
		soundEngine->PlayMusic("gamedata\\selectSong.mp3",0,true);
		soundEngine->PlaySound("gamedata\\Sweep.wav",systemIni.sndVolume);
	}
