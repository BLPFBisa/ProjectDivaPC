...
void ListBox::AddItem(string flag)
{
...
MainMenu::MainMenu()
{
	...
	//songlb->AddItem("恋愛サーキュレーション");
	//songlb->ReplaceItem(0,true);
	InitSongLb();
...
void MainMenu::Draw()
{
	...
	else if(MainMenuState==MAINST_FREE||MainMenuState==MAINST_SELECTSONG)
	{
		...
		if(MainMenuState==MAINST_FREE)
		{
			...
			graphEngine.DrawText("Fuck",MakeRect(0,223,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
...
void ScoreMenu::Draw()
{
	...
	graphEngine.SetFont("宋体",14);
	graphEngine.DrawText(MakeRect(141,87,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",coolcntN);
	graphEngine.DrawText(MakeRect(141,105,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",finecntN);
	graphEngine.DrawText(MakeRect(141,123,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",safecntN);
	graphEngine.DrawText(MakeRect(141,141,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",sadcntN);
	graphEngine.DrawText(MakeRect(141,159,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",worstcntN);
	graphEngine.DrawText(MakeRect(141,177,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",combocntN);
	graphEngine.DrawText(MakeRect(141,195,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%d",chancetimecntN);

	graphEngine.SetFont("宋体",18);
	graphEngine.DrawText(MakeRect(141,223,99,15),DT_RIGHT | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)),
					"%.7d",scorecntN);
...
void ScoreMenu::ResetResult(int coolCnt, int fineCnt, int safeCnt, int sadCnt, int worstCnt, int comboCnt, int chancetimeCnt, int scoreCnt, bool ifclear,int maxScoreCanGet)
{
	coolcnt=coolCnt;
	finecnt=fineCnt;
	safecnt=safeCnt;
...
BottomBar::BottomBar()
{
	...
	labelDisplay = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(346,0,32,10),MakeRect(0,0,32,10));
...
void BottomBar::MainMenuStateChanged(int MainMenuState)
{
	...
	if(MainMenuStateGoTo==MAINST_MAIN)
	{
	...
	else if(MainMenuStateGoTo==MAINST_FREE)
	{
		bottom2L = 480-33 -(labelMove->drawRect.right-labelMove->drawRect.left) - 16
				...
				if(hFindDiva!=INVALID_HANDLE_VALUE)
				{
					do
					{
						...
						if(isImage(thisFname))
						{
							...
							_cg.push_back(new GUIStaticImage(thisFname, MakeRect(0,0,480,272), MakeRect(0,0,0,0), 3));
						}
					}
					while(FindNextFile(hFindDiva,&wfdDiva));
					FindClose(hFindDiva);
