...
{
	title->Draw();
	graphEngine.SetFont("宋体",13,false,FW_BOLD);
...
void ListBox::AddItem(string flag,int style)
{
	...
	btns->GetButton(btns->GetButtonNum()-1)->SetText(flag,UserFont("宋体",12,false,FW_BOLD),MakeRect(16,8,0,0));
}
...
MainMenu::MainMenu()
{
	...
	songlb = new ListBox(MakeRect(0,0,0,0),"没有歌曲");
	InitSongLb();
	songlb->titleWords = "自由模式";
	...
	if(songlb->btns->GetButtonNum()<=0)
		mainlb->AddItem("自由模式（没有歌曲）");
	else
		mainlb->AddItem("自由模式");
	mainlb->AddItem("生涯剧情模式（未开启）");
	mainlb->AddItem("网络对战模式（未开启）");
	mainlb->AddItem("设置");
	mainlb->AddItem("CG鉴赏");
	mainlb->AddItem("其他信息");
	mainlb->AddItem("退出游戏");
	...
	mainlb->GetIn();
	mainlb->titleWords = "菜单";
	(*mainDetail->efvct.a) = 0;
	if(songlb->btns->GetButtonNum()<=0)
		mainDetailWords = "没有检测到\n演奏节奏游戏\n无法进入";
	else
		mainDetailWords = "可以\n演奏节奏游戏";
	MainMenuState = MAINST_MAIN;

	configlb = new ListBox(MakeRect(0,0,0,0),"I love miku");
	configlb->titleWords = "设置";
	configlb->AddItem("按键习惯");
	configlb->AddItem("窗口大小");
	configlb->AddItem("音效音量",1);
	configlb->AddItem("恢复默认设置");
	...
	else if(MainMenuState==MAINST_FREE||MainMenuState==MAINST_SELECTSONG)
	{
		...
		if(MainMenuState==MAINST_FREE)
		{
			if(songlb->btns->GetButtonNum()>0)
			{
				...
				graphEngine.DrawText("作者",MakeRect(0,158,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
...
void SelectSong::Draw()
{
	...
	if(ts!=-1)
		graphEngine.DrawText(MakeRect(356,150,0,0),DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(talpha,255,255,255)),
			"难度分类:%s\n难度星级:%d星\n\n  最高分:%.7d\n最高连击:%.4d\n最高评价:%s\n通关次数:%d",HardType[ts].c_str(),nowSong->hard[ts],
...
void GUIMessageBox::Draw()
{
	...
	if(prepareState==3)
	{
		...
		if(_basicInfo!="")
		{
			graphEngine.SetFont("宋体",fontSize);
