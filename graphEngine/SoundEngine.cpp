...
const float SoundEngine::soundSpd = 0.03f;
...
void SoundEngine::PlayHit()
{
	...
	result = system->playSound(FMOD_CHANNEL_FREE, hitWav, false, &channel);
	ERRCHECK(result);
	result = channel->setVolume(0.3f*systemIni.sndVolume);
	ERRCHECK(result);
}
...
void SoundEngine::PlayMusic(string filename, CHANNEL Ichannel, bool loop, float volume)
{
