...
bool GUIEF_Stretch::OnWork()
{
	if(!paused)
	{
		...
		_nowLife++;
		SetRect(_pRect,_beginRect.left + int(float(_desRect.left - _beginRect.left) * (float(_nowLife) / float(_maxLife))),
					_beginRect.top + int(float(_desRect.top - _beginRect.top) * (float(_nowLife) / float(_maxLife))),
					_beginRect.right + int(float(_desRect.right - _beginRect.right) * (float(_nowLife) / float(_maxLife))),
					_beginRect.bottom + int(float(_desRect.bottom - _beginRect.bottom) * (float(_nowLife) / float(_maxLife))));
		if(_nowLife==_maxLife)
			return false;
	}
	return true;
...
void GUIControl::OnKeyEvent(unsigned int nChar,bool bKeyDown,bool bAltDown)
{
	for(vector<GUIControl*>::iterator ptr = panel.begin();ptr!=panel.end();ptr++)
...
void GUIStaticImage::Draw(int Basex,int Basey)
{
	if(visible)
	{
		graphEngine.Draw(drawTex,srcRect,MakeRect(drawRect.left+Basex,drawRect.top+Basey,drawRect.right-drawRect.left,drawRect.bottom-drawRect.top),D3DCOLOR_ARGB(_a,_r,_g,_b));
...
void NormalButton::Draw(int Basex, int Basey)
{
	if(visible)
	{
		...
		if(text!="")
		{
			graphEngine.SetFont(font.ftFaceName,font.ftSize,font.ftItalic,font.ftWeight);
			graphEngine.DrawText(text,MakeRect(Basex+drawRect.left+ftDrawRect.left,Basey+drawRect.top+ftDrawRect.top,
				ftDrawRect.right-ftDrawRect.left,ftDrawRect.bottom-ftDrawRect.top),DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(_a,_r,_g,_b)));
		}
		GUIControl::Draw(Basex,Basey);
	}
}
...
void NormalButton::SetText(string _text,UserFont _font,RECT _ftDrawRect)
{
	text = _text;
	font = _font;
...
void NormalButtonGroup::OnKeyEvent(unsigned int nChar, bool bKeyDown, bool bAltDown)
{
	if(focusOn&&bKeyDown)
	{
		if(focus==-1)
...
ListBox::ListBox(RECT drawRect,string noItem)
{
	...
	title = new GUIStaticImage("pic\\menu_basic.png",MakeRect(0,0,164,17),MakeRect(0,32,164,17));
	titleWords = "";
...
void ListBox::OnKeyEvent(unsigned int nChar, bool bKeyDown, bool bAltDown)
{
...
{
	title->Draw();
	graphEngine.SetFont("ＭＳ 明朝",13,false,FW_BOLD);
	graphEngine.DrawText(titleWords,title->drawRect.left+8,title->drawRect.top+3);
	btns->Draw();
	up->Draw();
	down->Draw();
}
...
void ListBox::AddItem(string flag,int style)
{
	if(style==0)
	{
		btns->AddButton(new NormalButton(MakeRect(0,0,204,17),"pic\\menu_basic.png",MakeRect(0,17,204,17),MakeRect(0,0,204,17),
					"pic\\menu_basic04.png",MakeRect(0,97,206,31),MakeRect(0,-7,206,31),
					"pic\\menu_basic.png",MakeRect(0,17,204,17),MakeRect(0,0,0,0),"pic\\menu_basic.png",MakeRect(0,17,204,17),MakeRect(0,0,0,0)));
	}
	else
	{
		btns->AddButton(new NormalButton(MakeRect(0,0,204,17),"pic\\menu_basic.png",MakeRect(0,34,204,17),MakeRect(0,0,204,17),
					"pic\\menu_basic04.png",MakeRect(0,31,206,31),MakeRect(0,-7,206,31),
					"pic\\menu_basic.png",MakeRect(0,34,204,17),MakeRect(0,0,0,0),"pic\\menu_basic.png",MakeRect(0,34,204,17),MakeRect(0,0,0,0)));
	}
	btns->GetButton(btns->GetButtonNum()-1)->SetText(flag,UserFont("ＭＳ 明朝",12,false,FW_BOLD),MakeRect(16,8,0,0));
}
...
MainMenu::MainMenu()
{
	top1 = new GUIStaticImage("pic\\menu_title_home.png",MakeRect(95,0,417,21),MakeRect(63,0,417,21));
	top2 = new GUIStaticImage("pic\\menu_title_home.png",MakeRect(32,21,480,55),MakeRect(0,0,480,55));
	title = new GUIStaticImage("pic\\menu_title_home.png",MakeRect(389,76,123,34),MakeRect(0,0,123,34));

	bottom = new BottomBar();
	
	songlb = new ListBox(MakeRect(0,0,0,0),"自由モード");
	InitSongLb();
	songlb->titleWords = "自由モード";

	//back = new GUIStaticImage("pic\\adhoc_01.png",MakeRect(16,120,480,272),MakeRect(0,0,480,272));
	back = new MainMenuBack();
	mainDetail = new GUIStaticImage("pic\\menu_common.png",MakeRect(335,115,177,116),MakeRect(297,50,177,116));
	mainlb = new ListBox(MakeRect(0,0,0,0),"FUCKSHIT");
	if(songlb->btns->GetButtonNum()<=0)
		mainlb->AddItem("自由モード(音楽が見つかりません)");
	else
		mainlb->AddItem("自由モード");
	mainlb->AddItem("キャンペーンモード(無効)");
	mainlb->AddItem("ネットワークモード(無効)");
	mainlb->AddItem("設定");
	mainlb->AddItem("CG鑑賞");
	mainlb->AddItem("別の情報");
	mainlb->AddItem("ゲームを終了");
	mainlb->ReplaceItem(0,true);
	mainlb->GetIn();
	mainlb->titleWords = "メニュー";
	(*mainDetail->efvct.a) = 0;
	if(songlb->btns->GetButtonNum()<=0)
		mainDetailWords = "リズムが検出されません\nゲームを読み込んでいません";
	else
		mainDetailWords = "リズムのゲームを演奏ます";
	MainMenuState = MAINST_MAIN;

	configlb = new ListBox(MakeRect(0,0,0,0),"I love miku");
	configlb->titleWords = "設定";
	configlb->AddItem("クラヴィーア設定");
	configlb->AddItem("ウィンドウのサイズ");
	configlb->AddItem("オーディオの音量",1);
	configlb->AddItem("デフォルトの設定");
	...
	songPicBack = new GUIStaticImage("pic\\menu_common.png",MakeRect(187,9,169,100),MakeRect(296,50,169,100));
	(*songPicBack->efvct.a) = 0;
	blackBack = new GUIStaticImage("pic\\menu_common.png",MakeRect(0,0,186,205),MakeRect(292,44,186,205));
	(*blackBack->efvct.a) = 0;
...
void MainMenu::Draw()
{
	...
	{
		mainlb->Draw();
		graphEngine.SetFont("ＭＳ 明朝",12);
		graphEngine.DrawText(mainDetailWords,307,105,D3DXCOLOR(D3DCOLOR_ARGB((*mainDetail->efvct.a),255,255,255)));
	}
	else if(MainMenuState==MAINST_FREE||MainMenuState==MAINST_SELECTSONG)
	{
		...
		if(MainMenuState==MAINST_FREE)
		{
			if(songlb->btns->GetButtonNum()>0)
			{
				...
				graphEngine.DrawText("著者",MakeRect(0,158,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				...
				graphEngine.DrawText("Easy",MakeRect(0,171,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				graphEngine.DrawText(songInfo.songs[songlb->btns->GetSelectedIndex()].hardText[0],MakeRect(340,171,0,13),D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));

				graphEngine.DrawText("Normal",MakeRect(0,184,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				graphEngine.DrawText(songInfo.songs[songlb->btns->GetSelectedIndex()].hardText[1],MakeRect(340,184,0,13),D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));

				graphEngine.DrawText("Hard",MakeRect(0,197,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				graphEngine.DrawText(songInfo.songs[songlb->btns->GetSelectedIndex()].hardText[2],MakeRect(340,197,0,13),D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));

				graphEngine.DrawText("Extra",MakeRect(0,210,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				graphEngine.DrawText(songInfo.songs[songlb->btns->GetSelectedIndex()].hardText[3],MakeRect(340,210,0,13),D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));

				graphEngine.DrawText("Die",MakeRect(0,223,335,13),DT_RIGHT | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
				graphEngine.DrawText(songInfo.songs[songlb->btns->GetSelectedIndex()].hardText[4],MakeRect(340,223,0,13),D3DXCOLOR(D3DCOLOR_ARGB((*blackBack->efvct.a),255,255,255)));
			}
		}
		else
		{
	...
	else if(MainMenuState==MAINST_CONFIG)
	{
		configlb->Draw();
		graphEngine.SetFont("ＭＳ 明朝",12);
		graphEngine.DrawText(configDetailWords,307,93,D3DXCOLOR(D3DCOLOR_ARGB((*mainDetail->efvct.a),255,255,255)));
	}
...
void MainMenu::OnKeyEvent(unsigned int nChar, bool bKeyDown, bool bAltDown)
{
	if(MainMenuState==MAINST_MAIN)
	{
...
void MainMenu::InitSongLb()
{
	...
	for(int i=0;i<songInfo.songs.size();i++)
		songlb->AddItem(songInfo.songs[i].songName);
			...
			if(wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				if(hFindDiva!=INVALID_HANDLE_VALUE)
				{
					do
					{
						...
						tinfo.songFNames.push_back(thisFname);
						tinfo.songFNameBase = szFileDirec;
					}
...
SelectSong::SelectSong()
{
	...
	btns->AddButton(new NormalButton(MakeRect(300,143,48,17),"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
	...
	btns->AddButton(new NormalButton(MakeRect(300,162,48,17),"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(201,229,59,31),MakeRect(-5,-2,59,31),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17)));

	btns->AddButton(new NormalButton(MakeRect(300,181,48,17),"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(201,229,59,31),MakeRect(-5,-2,59,31),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17)));

	btns->AddButton(new NormalButton(MakeRect(300,200,48,17),"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(201,229,59,31),MakeRect(-5,-2,59,31),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17)));

	btns->AddButton(new NormalButton(MakeRect(300,219,48,17),"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(201,229,59,31),MakeRect(-5,-2,59,31),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17),
							"pic\\menu_common.png",MakeRect(268,231,48,17),MakeRect(0,0,48,17)));
...
void SelectSong::OnKeyEvent(unsigned int nChar,bool bKeyDown,bool bAltDown)
{
	if(sleep)
		return;
	int tfocus = btns->GetSelectedIndex();
...
{
	btns->Draw();
	graphEngine.SetFont("ＭＳ 明朝",14);
	int talpha=255;
	if(alphaHook)
		talpha=(*alphaHook);
	for(int i=0;i<5;i++)
		if(btns->GetButton(i)->GetButtonState()!=BTNSTATE_DISABLE)
			graphEngine.DrawText(HardType[i],btns->GetButton(i)->drawRect,DT_NOCLIP | DT_CENTER | DT_VCENTER,D3DXCOLOR(D3DCOLOR_ARGB(talpha,255,255,255)));
		else
			graphEngine.DrawText(HardType[i],btns->GetButton(i)->drawRect,DT_NOCLIP | DT_CENTER | DT_VCENTER,D3DXCOLOR(D3DCOLOR_ARGB(talpha,100,100,100)));
	graphEngine.SetFont("ＭＳ 明朝",12);
	...
	if(ts!=-1)
		graphEngine.DrawText(MakeRect(356,150,0,0),DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(talpha,255,255,255)),
			"  難易度:%s\n難易度のレベル:%d\n\n 最高記録:%.7d\n最高combo:%.4d\n 最高評価:%s\nクリアしの回:%d",HardType[ts].c_str(),nowSong->hard[ts],
			nowSong->highScore[ts],nowSong->maxCombo[ts],SongResult[nowSong->highResult[ts]].c_str(),nowSong->clearNum[ts]);
}
...
ScoreMenu::ScoreMenu()
{
	...
	scoreBack = new GUIStaticImage("pic\\result_win_nm.png",MakeRect(0,0,240,212),MakeRect(10,30,240,212));
	...
	resultWord=NULL;
	btns = new NormalButtonGroup(MakeRect(0,0,0,0));
	btns->AddButton(new NormalButton(MakeRect(291,225,69,17),"pic\\menu_result.png",MakeRect(290,23,69,17),MakeRect(0,0,69,17),
						...
						"pic\\menu_result.png",MakeRect(290,23,69,17),MakeRect(0,0,69,17)));
	btns->AddButton(new NormalButton(MakeRect(372,225,69,17),"pic\\menu_result.png",MakeRect(290,23,69,17),MakeRect(0,0,69,17),
						"pic\\menu_result.png",MakeRect(202,142,81,21),MakeRect(-5,-2,81,21),
						"pic\\menu_result.png",MakeRect(290,23,69,17),MakeRect(0,0,69,17),
						"pic\\menu_result.png",MakeRect(290,23,69,17),MakeRect(0,0,69,17)));
	btns->Focus();
...
void ScoreMenu::OnKeyEvent(unsigned int nChar, bool bKeyDown, bool bAltDown)
{
...
void ScoreMenu::Draw()
{
	...
	graphEngine.SetFont("ＭＳ 明朝",12);
	graphEngine.DrawText(_songInfo,MakeRect(scoreBack->drawRect.left,scoreBack->drawRect.top + 26,scoreBack->drawRect.right-scoreBack->drawRect.left,24),
		DT_CENTER | DT_VCENTER | DT_NOCLIP, D3DCOLOR_ARGB(255,255,255,255));

	graphEngine.SetFont("ＭＳ 明朝",14);
	
	numberMana.Draw(coolcntN,MakeRect(141,87,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(finecntN,MakeRect(141,105,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(safecntN,MakeRect(141,123,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(sadcntN,MakeRect(141,141,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(worstcntN,MakeRect(141,159,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(combocntN,MakeRect(141,177,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);
	numberMana.Draw(chancetimecntN,MakeRect(141,195,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),0);

	numberMana.Draw(scorecntN,MakeRect(141,223,99,15),NumberStyle::SCORE, NumberDwFlags::RIGHT,D3DCOLOR_ARGB(255,255,255,255),2,7);

	if(menust==SCOREMENUST_OVER)
	{
		graphEngine.SetFont("ＭＳ 明朝",12);
		graphEngine.DrawText("RETRY",btns->GetButton(0)->drawRect,DT_CENTER | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)));
		graphEngine.DrawText("RETURN",btns->GetButton(1)->drawRect,DT_CENTER | DT_VCENTER | DT_NOCLIP,D3DXCOLOR(D3DCOLOR_ARGB(255,255,255,255)));
	}
}
...
void ScoreMenu::Update()
{
	...
	{
		bool flag=false;
		GetOnValue(coolcntN,coolcnt,8,flag);
		GetOnValue(finecntN,finecnt,8,flag);
		GetOnValue(safecntN,safecnt,8,flag);
		GetOnValue(sadcntN,sadcnt,8,flag);
		GetOnValue(worstcntN,worstcnt,8,flag);
		GetOnValue(combocntN,combocnt,8,flag);
		GetOnValue(chancetimecntN,chancetimecnt,8,flag);
		GetOnValue(scorecntN,scorecnt,8,flag);
		if(!flag)
		{
...
void ScoreMenu::ResetResult(int coolCnt, int fineCnt, int safeCnt, int sadCnt, int worstCnt, int comboCnt, int chancetimeCnt, int scoreCnt, bool ifclear,int maxScoreCanGet,string songInfo)
{
	...
	scorecnt=scoreCnt;
	ifClear=ifclear;
	coolcntN=finecntN=safecntN=sadcntN=worstcntN=combocntN=chancetimecntN=scorecntN=0;
...
{
	backColor = new GUIStaticImage("pic\\menu_back.png",MakeRect(0,0,480,272),MakeRect(0,0,480,272));
	circleOut = new GUIStaticImage("pic\\menu_com.png",MakeRect(325,0,699,697),MakeRect(355,0,175,175));//530 135 ﾔｲﾐﾄ 350
...
BottomBar::BottomBar()
{
	bottom1 = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(28,179,480,16),MakeRect(0,256,480,16));
	bottom2 = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(0,117,480,25),MakeRect(480,247,480,25));
	bottom2L = 480;

	labelMove = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(309,33,35,10),MakeRect(0,0,35,10));
	labelSelect = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(0,0,36,10),MakeRect(0,0,36,10));

	labelPV = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(371,22,46,10),MakeRect(0,0,46,10));
	labelDisplay = new GUIStaticImage("pic\\menu_basic02.png",MakeRect(346,0,34,10),MakeRect(0,0,34,10));
...
CGMenu::CGMenu()
{
	...
	_title = new GUIStaticImage("pic\\menu_title_home.png",MakeRect(260,76,123,34),MakeRect(0,0,123,34));

	_cgBack = new GUIStaticImage("pic\\menu_common.png",MakeRect(193,9,147,82),MakeRect(0,0,0,0));
	...
	_lArrow = new GUIStaticImage("pic\\menu_basic04.png", MakeRect(217,18,13,13), MakeRect(6,136-13,26,26));
	_rArrow = new GUIStaticImage("pic\\menu_basic04.png", MakeRect(230,18,13,13), MakeRect(448,136-13,26,26));
...
void CGMenu::OnKeyEvent(unsigned int nChar, bool bKeyDown, bool bAltDown)
{
...
void CGMenu::Draw()
{
	...
	if(_cg.size()!=0)
	{
		graphEngine.SetFont("Arial", 14);
		graphEngine.DrawText(_pageNumBack->GetDrawRect(), DT_CENTER | DT_VCENTER, "No. %d / %d", nowSelected+1, _cg.size());
		graphEngine.DrawText("Illusion: " + cgFileInfo._illusion[nowSelected], _illusionBack->GetDrawRect(), DT_CENTER | DT_VCENTER);
	}
...
void GUIMessageBox::PrepareMenu(string basicInfo,int drawWidth)
{
...
void GUIMessageBox::AddItem(string option)
{
	if(prepareState!=0)
		return;
...
void GUIMessageBox::ShowMenu()
{
	...
	showRect = MakeRect((base::Width-_drawWidth)/2,(base::Height-drawHeight)/2,_drawWidth,drawHeight);
	drawRect = 
...
void GUIMessageBox::Draw()
{
	...
	if(prepareState==3)
	{
		...
		if(_basicInfo!="")
		{
			graphEngine.SetFont("ＭＳ 明朝",fontSize);
			graphEngine.DrawText(_basicInfo, MakeRect(drawRect.left,tpos,drawRect.right-drawRect.left,fontSize), DT_CENTER,
		...
		if(_option.size()!=0)
		{
			for(int i=0;i<_option.size();i++)
			{
				...
				graphEngine.DrawText(_option[i], MakeRect(drawRect.left,tpos,drawRect.right-drawRect.left,fontSize), DT_CENTER,
					D3DCOLOR_ARGB(255,255,255,255));
				tpos+=fontSize;
			}
		}
...
void GUIMessageBox::OnKeyEvent(unsigned int nChar,bool bKeyDown,bool bAltDown)
{
			...
			if(wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				if(hFindDiva!=INVALID_HANDLE_VALUE)
				{
					do
					{
						...
						{
							_illusion.push_back(wfd.cFileName);
