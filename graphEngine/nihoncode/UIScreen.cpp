...
void menu_configlb_RefreshDetail()
{
	...
	menu->configlb->btns->GetButton(0)->SetText("クラヴィーア設定" + KeyType,UserFont("ＭＳ 明朝",12,false,FW_BOLD),MakeRect(16,8,0,0));
	menu->configlb->btns->GetButton(1)->SetText("ウィンドウのサイズ:" + ToString(systemIni.winWidth) + "*" + ToString(systemIni.winHeight),UserFont("ＭＳ 明朝",12,false,FW_BOLD),MakeRect(16,8,0,0));
	menu->configlb->btns->GetButton(2)->SetText("SE音量          " + ToString(systemIni.soundVolume) + "/10",UserFont("ＭＳ 明朝",12,false,FW_BOLD),MakeRect(16,8,0,0));
...
void menu_mainlb_OnSelectedIndexChanged(int id)
{
	...
	{
		if(menu->songlb->btns->GetButtonNum()<=0)
			menu->mainDetailWords = "リズムが検出されません\nゲームを読み込んでいません";
		else
			menu->mainDetailWords = "リズムのゲームを演奏ます";
	}
	else if(id==1)
		menu->mainDetailWords = "みくちゃんと一緒に\nLIVEを始まりますよ\nまだ現在開いている";
	else if(id==2)
		menu->mainDetailWords = "ネットバトル\nまだ現在開いている";
	else if(id==3)
		menu->mainDetailWords = "オプション";
	else if(id==4)
		menu->mainDetailWords = "ＣＧ館と映画館";
	else if(id==5)
		menu->mainDetailWords = "GameMaster.\"Project Z1\"\n非オリジナルの素材\nは株式会社セガに属している";
	else if(id==6)
		menu->mainDetailWords = "ゲームの終了";
...
void RefreshConfigDetail()
{
	...
	{
		menu->configDetailWords = "変更ウィンドウのサイズ:\n480*272\n720*408\n960*544";
	}
...
void MainFirstInit()
{
	...
	pausedmenu->AddItem("続行");
	pausedmenu->AddItem("再試行");
	pausedmenu->AddItem("あきらめる");
